    ...
    const redirectToCheckout = async (e, _lineItems) => {
        ...
        if (_lineItems.length > 0) {
            ...
            return await apolloClient.mutate({
                mutation: gql`
                mutation Mutation($checkoutCreateInput: CheckoutCreateInput!) {
            ...
            `,
                variables: { checkoutCreateInput: { lineItems: _lineItems } }
            })
                .then((checkout) => {
                    router.push(checkout.data.checkoutCreate.checkout.webUrl)
