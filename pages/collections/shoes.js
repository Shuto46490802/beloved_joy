...
const Shoes = ({ shoesData, products, headerRef, humburgerRef, }) => {

    const viewAll = products.filter(({ category }) => category.indexOf("shoes") !== -1);
    const sneakers = products.filter(({ category }) => category.indexOf("sneaker") !== -1);
    const boots = products.filter(({ category }) => category.indexOf("boots") !== -1);
    const flats = products.filter(({ category }) => category.indexOf("flats") !== -1);
    const sandals = products.filter(({ category }) => category.indexOf("sandals") !== -1);
